...
df <- readHTMLTable("http://games.espn.go.com/ffl/tools/projections?&seasonTotals=true&seasonId=2015&scoringPeriodId=8")

espn_scrape <- function(proj_year, proj_week) {
  base_url <- paste0('http://games.espn.go.com/ffl/tools/projections?',
                     'seasonId=', proj_year,
                     '&scoringPeriodId=', proj_week)
  readHTMLTable(base_url, stringsAsFactors = FALSE, header = TRUE, skip.rows = 1)
}
...
espn_urls <- function() {
  season <- 2015
  week <- 8
  pos <- list(QB=0, RB=2, WR=4, TE=6, K=17, DST=16)
  pages <- c("0","40","80")
  base_url = paste0("http://games.espn.go.com/ffl/tools/projections?&seasonTotals=true&seasonId=", season, "&scoringPeriodId=", week)
  urls <- paste0(base_url, "&slotCategoryId=", rep(pos, each=length(pages)), "&startIndex=", pages)
  lapply(urls, function(x){readHTMLTable(x, stringsAsFactors=FALSE, skip.rows = 1)$playertable_0})
}

# download ESPN

espn_base_url = paste0("http://games.espn.go.com/ffl/tools/projections?&seasonTotals=true&seasonId=", season, "&scoringPeriodId=", week)
